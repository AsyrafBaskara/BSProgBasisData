...
if($_SESSION['idlog']==$id || isitok($_SESSION['idlog'])){
  if($id==1){
  ...
  echo"<script>alert('you may pass')</script>";
}else{
  header('location:../index.php');
  exit;
}
...
$whodata= query("SELECT * FROM tman WHERE tman_id =$id")[0];

$password=$whodata["password"];
...
if(isset($_POST['enter'])){
  if($_POST['oldpassword']===""&&$_POST['newpassword']===""){
    ...
    if(shmanc($_POST)){
      echo"<script>alert('TrashMan Edit Successful');</script>";
      header("Location:../maindata.php");
    } else {
      echo"<script>alert('TrashMan Edit Failed');</script>";
      header("Location:../maindata.php");
    }
  } else if($_POST["newpassword"]!==""&&$_POST["oldpassword"]!==""){//buatpassbaru
    if (password_verify($_POST["oldpassword"],$password)){
      $_POST["oldpassword"] = password_hash($_POST["newpassword"], PASSWORD_DEFAULT);
    if(shmanc($_POST)){
    echo"<script>alert('TrashMan Edit Successful');</script>";
    header("Location:../maindata.php");
  } else {
  ...
  echo"<script>alert('Error1 Found');</script>";
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Bank Sampah | Trash Man</title>
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">TrashMan's Edit Page</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Trashman Page</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
      ...
      <div class="container-fluid">
        ...
        <div class="login">
              ...
              <form method="post" action="" autocomplete="off" class="formbox">
                <input type="hidden" name="tman_id" value="<?= $whodata["tman_id"]; ?>">
                <b><span class="spanform">Name</span></b>
                <input type="text" class="text" name="tman_name" value="<?= $whodata["tman_name"]; ?>">
                <b><span class="spanform">Email</span></b>
                <input type="email" class="text" name="email" value="<?= $whodata["email"]; ?>">
                <br>
                <em style="font-size:8px;color:red;">*Don't input if you won't change the password</em>
                <br>
                <b><span class="spanform">Old Password</span></b>
                <input type="password" class="text" name="oldpassword">
                <b><span class="spanform">New Password</span></b>
                <input type="password" class="text" name="newpassword">
