...
$whodata= query("SELECT * FROM trash WHERE trash_id =$id")[0];
$result = query("SELECT * FROM ttype");

if(isset($_POST['enter'])){
    if(shc($_POST)){
      echo"<script>alert('Trash Edit successful');</script>";
      header("Location:../maindata.php");
    } else {
      echo"<script>alert('Trash Edit Failed');</script>";
      header("Location:../maindata.php");
    }
...
<head>
  ...
  <title>Bank Sampah | Trasher Edit</title>
      ...
      <div class="container-fluid">
        ...
        <div class="login">
              ...
              <form method="post" action="" autocomplete="off" class="formbox">
                <input type="hidden" name="trash_id" value="<?= $whodata["trash_id"]; ?>">
                <b><span class="spanform">Trash Name</span></b>
                <input type="text" class="text" name="name" value="<?= $whodata["trash_name"]; ?>">
                <b><span class="spanform">Type</span></b>
                <select name="trashtype" class="option" style="">

                <option selected="true" disabled="disabled">--Choose--<em>before: <?= $whodata["trash_type"]; ?></em></option>
                  <?php
                  foreach($result as $dttype){ ?>
                    <option value="<?=$dttype['trash_type']?>"><?=$dttype['trash_type']?></option>
                  <?php }
                  ?>
                </select>

                <b><span class="spanform">Price/Kg</span></b><br>
                <b style="display:inline-block">Rp</b><input type="number" class="text" name="price" min="1000" step="1000" max="1000000" value="<?= $whodata["price"]; ?>" />
